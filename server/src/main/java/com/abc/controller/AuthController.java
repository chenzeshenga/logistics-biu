 ...
 */
@PermInfo(value = "登录模块", pval = "a:auth") @RestController @RequestMapping("/auth") public class AuthController {
    ...
    @PostMapping("/login") public Json login(@RequestBody String body) {
        ...
        try {
            ...
            return Json.succ(oper).data("token", UUID.randomUUID().toString()).data("uid", user.getUid())
                .data("nick", user.getNick()).data("roles", user.getRoles()).data("perms", user.getPerms());
    ...
    @PostMapping("/logout") public Json logout() {
    ...
    @GetMapping("/info") public Json info() {
        ...
        if (user == null) {
            ...
            //返回登录用户的信息给前台，含用户的所有角色和权限
            return Json.succ(oper).data("name", user.getUname()).data("nick", user.getNick()).data("avator", "")
