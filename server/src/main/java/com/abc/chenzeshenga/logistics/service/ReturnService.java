...
public class ReturnService extends ServiceImpl<ReturnMapper, Return> {
    ...
    public ReturnService(
        ReturnOrdContentService returnOrdContentService, UserCommonService userCommonService) {
    ...
    public PageData<Return> listV2(PageQueryEntity<Return> returnPageQueryEntity) {
        ...
        List<Return> returnList =
            returnMapper.listV2(returnEntity, SqlUtils.generateSqlLimit(pagination));
        returnList.forEach(
            returnEle -> {
                returnEle.setToDetailAddressLbl(
                    StringUtils.stripToEmpty(returnEle.getToAddressLine1())
                        + "/"
                        + StringUtils.stripToEmpty(returnEle.getToAddressLine2())
                        + "/"
                        + StringUtils.stripToEmpty(returnEle.getToAddressLine3()));
                returnEle.setFromDetailAddressLbl(
                    StringUtils.stripToEmpty(returnEle.getFromAddressLine1())
                        + "/"
                        + StringUtils.stripToEmpty(returnEle.getFromAddressLine2())
                        + "/"
                        + StringUtils.stripToEmpty(returnEle.getFromAddressLine3()));
                String returnNo = returnEle.getReturnNo();
                List<ReturnContent> returnContentList = returnMapper.selectContentById(returnNo);
                returnEle.setContentList(returnContentList);
                List<ClaimContentDealing> claimContentDealingList = returnMapper.selectContentDealingById(returnNo);
                returnEle.setContentDealingList(claimContentDealingList);
                List<ClaimPackage> claimPackageList = returnMapper.selectClaimPackageById(returnNo);
                returnEle.setClaimPackageList(claimPackageList);
            });
    ...
    /**
     * 每天零点10分运行定时任务 将创建时间在 14天的退货单的状态更新为 历史
     */
    @PostConstruct
    ...
    public void updateClaimContentDealing(List<ClaimContentDealing> claimContentDealingList) {
        ...
        claimContentDealingList.forEach(
            claimContentDealing -> {
