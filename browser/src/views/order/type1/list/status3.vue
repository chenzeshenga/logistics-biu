...
<script>
  ...
  export default {
    ...
    methods: {
      reRouter() {
        this.$router.push({
          path: '/order-list/mgt/type1/status3',
        });
      },
      onQuickSearch() {
      ...
      searchOrd() {
        ...
        if (this.daterange == null || this.daterange[0] === 0 || this.daterange[1] === 0) {
          ...
          url = 'ord/list/1/3/2000-01-01/2099-01-01?ordno=' + this.search.ordno + '&creator=' + this.search.creator +
            '&channelCode=' + this.search.channelCode;
        } else {
          url = 'ord/list/1/3/' + this.daterange[0] + '/' + this.daterange[1] + '?ordno=' + this.search.ordno + '&creator=' + this.search.creator +
            '&channelCode=' + this.search.channelCode;
        }
        this.tableLoading = true;
        request({
