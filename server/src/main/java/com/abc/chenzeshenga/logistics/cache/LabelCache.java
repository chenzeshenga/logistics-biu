...
public class LabelCache {
    ...
    /**
     * @param keyPattern would be like sth like "label_", the underline "_" is
     *                   required
     * @return
     */
    public List<Label> getLabelList(String keyPattern) {
        ...
        for (Map.Entry<String, String> entry : labelCache.entrySet()) {
            if (entry.getKey().startsWith(keyPattern)) {
                labelList.add(new Label(0, Integer.valueOf(entry.getKey().replace(keyPattern, "")), entry.getKey(),
