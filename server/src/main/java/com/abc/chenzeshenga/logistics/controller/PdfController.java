...
public class PdfController {
  ...
  @GetMapping("/ord/{ordno}")
  public void pdf(HttpServletResponse response, @PathVariable String ordno) throws IOException {
  ...
  private void generatePdfV1(
      @PathVariable String ordno, List<ManualOrderContent> manualOrderContentList, PdfWriter writer)
      throws IOException {
    try (PdfDocument pdf = new PdfDocument(writer)) {
      try (Document document = new Document(pdf, PageSize.A7)) {
        ...
        Paragraph head =
            new Paragraph().add("东岳物流").setFont(font).addStyle(new Style().setMarginLeft(30));
        document.add(head);
        Paragraph time =
            new Paragraph()
                .add(DateUtil.getStrFromDate(new Date()))
                .addStyle(new Style().setMarginLeft(50));
        document.add(time);
        Image barcode = new Image(ImageDataFactory.create(BarCodeUtil.generate(ordno, "code128")));
        Paragraph paragraph =
            new Paragraph()
                .add(barcode)
                .addStyle(new Style().setVerticalAlignment(VerticalAlignment.MIDDLE))
                ...
                table
                    .addCell(
                        new Cell()
                            .add(
                                new Paragraph(
                                    StringUtil.correctString(manualOrderContent.getName())))
                            .setFont(font))
                    .addCell(
                        new Cell()
                            .add(
                                new Paragraph(
                                    StringUtil.correctString(manualOrderContent.getLocation())))
                            .setFont(font))
                    .addCell(
                        new Cell()
                            .add(
                                new Paragraph(
                                    StringUtil.correctString(manualOrderContent.getNum())))
                            .setFont(font)));
        document.add(table);
      }
    }
  ...
  private void generatePdfV2(
    ...
    try (PdfDocument pdf = new PdfDocument(writer)) {
      try (Document document = new Document(pdf, new PageSize(141, 141))) {
        ...
        Paragraph head =
            new Paragraph().add("東岳配信QRコード").setFont(font).addStyle(new Style().setMarginLeft(15));
        document.add(head);
        Image barcode =
            new Image(
                ImageDataFactory.create(
                    BarCodeUtil.generate("http://www.jpdyu.com/#/order-info?ord=" + ordno)));
        Paragraph paragraph = new Paragraph().add(barcode).addStyle(new Style().setMarginLeft(30));
        document.add(paragraph);
        Paragraph ordnoTitle =
            new Paragraph()
                .add(new Text("お問い合わせ番号: " + ordno).setFontSize(6))
                .setFont(font)
  ...
  @GetMapping("/sku/{dySku}")
  public void skuPdf(HttpServletResponse response, @PathVariable String dySku) throws IOException {
    response.setContentType("application/pdf");
