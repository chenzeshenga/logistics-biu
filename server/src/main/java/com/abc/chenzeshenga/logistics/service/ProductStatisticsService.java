...
import com.abc.chenzeshenga.logistics.model.sys.JobHis;
...
import com.abc.chenzeshenga.logistics.service.sys.JobHisService;
...
@Service
@Slf4j
public class ProductStatisticsService extends ServiceImpl<ProductStatisticsMapper, ProductStatistics> {
    ...
    public ProductStatisticsService(ProductService productService, CompanyProfileService companyProfileService, JobHisService jobHisService,
        ...
        this.jobHisService = jobHisService;
        this.pool = pool;
    }
    ...
    private void recordJobEndTime(JobHis currJobHis) {
        currJobHis.setEndTime(new Date());
        jobHisService.updateById(currJobHis);
    }

    private JobHis recordJobStartTime() {
        JobHis currJobHis = new JobHis();
        String uuid = SnowflakeIdWorker.generateStrId();
        currJobHis.setUuid(uuid);
        currJobHis.setUserId("sys");
        currJobHis.setJobName("daily_product_商品仓储体积按用户统计");
        currJobHis.setStartTime(new Date());
        jobHisService.insert(currJobHis);
        return currJobHis;
    }
