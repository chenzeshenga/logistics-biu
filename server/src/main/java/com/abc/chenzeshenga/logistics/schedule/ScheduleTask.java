...
public class ScheduleTask {
  ...
  public void countProduct() {
    ...
    List<Product> productList =
        productMapper.listByStatus(new Page(0, 999), "1", null, null, null, null);
    ...
    for (Product product : productList) {
      ...
      productStatisticsMap.put(
          product.getSku() + "/" + product.getDySku() + ":" + product.getCreatedBy(),
          productStatistics);
    }
                  ...
                  manualOrderContent -> {
                    ...
                    if (productStatistics == null) {
                      ...
                      productStatistics.setPendingPickupNum(
                          String.valueOf(
                              Integer.valueOf(manualOrderContent.getNum())
                                  - Integer.valueOf(manualOrderContent.getPicked())));
                      productStatistics.setPendingDeliverNum(
                          String.valueOf(Integer.valueOf(manualOrderContent.getPicked())));
                    } else {
                      productStatistics.setStatisticalTime(new Date());
                      ...
                      if (!StringUtils.isBlank(productStatistics.getPendingPickupNum())) {
                        pendingPickupNum =
                            String.valueOf(
                                Integer.valueOf(pendingPickupNum)
                                    + Integer.valueOf(productStatistics.getPendingPickupNum()));
                      }
                      productStatistics.setPendingPickupNum(pendingPickupNum);
                      String pendingDeliverNum =
                          String.valueOf(Integer.valueOf(manualOrderContent.getPicked()));
                      if (!StringUtils.isBlank(productStatistics.getPendingDeliverNum())) {
                        pendingDeliverNum =
                            String.valueOf(
                                Integer.valueOf(pendingDeliverNum)
                                    + Integer.valueOf(productStatistics.getPendingDeliverNum()));
                      }
    ...
    if (!productStatisticsList.isEmpty()) {
      productStatisticsList =
          productStatisticsList.stream()
              .filter(
                  productStatistics ->
                      (StringUtils.isNotBlank(productStatistics.getSku())
                              && StringUtils.isNotBlank(productStatistics.getDysku()))
                          && StringUtils.isNotBlank(productStatistics.getOwner()))
