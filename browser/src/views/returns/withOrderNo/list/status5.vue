        ...
        <el-table-column label="操作" width="250" fixed="right">
          <template slot-scope="scope">
            <el-tooltip content="恢复" placement="top">
              <el-button @click="handleUpdate(scope.$index,scope.row)" size="small" type="info" icon="el-icon-refresh" circle
                         plain></el-button>
            </el-tooltip>
...
<script>
  ...
  export default {
    ...
    methods: {
      reRouter() {
        this.$router.push({
          path: '/order-list/mgt/type1/status5',
        });
      },
      onQuickSearch() {
      ...
      handleUpdate(index, row) {
        this.$confirm('您确定要恢复该订单？', '提示', confirm).then(() => {
          request({
            url: 'ord/update/1/' + row.orderNo + '/1',
            method: 'get',
          }).then(res => {
            console.log(res);
            this.fetchData();
      ...
      searchOrd() {
        ...
        if (this.daterange == null || this.daterange[0] === 0 || this.daterange[1] === 0) {
          ...
          url = 'ord/list/1/5/2000-01-01/2099-01-01?ordno=' + this.search.ordno + '&creator=' + this.search.creator +
            '&channelCode=' + this.search.channelCode;
        } else {
          url = 'ord/list/1/5/' + this.daterange[0] + '/' + this.daterange[1] + '?ordno=' + this.search.ordno + '&creator=' + this.search.creator +
            '&channelCode=' + this.search.channelCode;
        }
        this.tableLoading = true;
        request({
