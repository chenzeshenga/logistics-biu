...
public class SysPermController {
  ...
  public Json listAllPermission() {
    ...
    if (list.isEmpty()) {
      ...
      Map<Integer, List<SysPerm>> permMap =
          list.stream().collect(Collectors.groupingBy(SysPerm::getPtype));
      ...
      if (buttonPermList != null && !buttonPermList.isEmpty()) {
        buttonsGroupedByParent =
            buttonPermList.stream().collect(Collectors.groupingBy(SysPerm::getParent));
      }
  ...
  public Json add(@RequestBody String body) {
    ...
    return Json.result(oper, success).data("created", perm.getCreated());
  }
  ...
  public Json listApiPermMetadata() {
    ...
    List<SysPerm> apiList =
        beans.stream()
            .filter(b -> StringUtils.equals(basicPackage, ClassUtils.getPackageName(b.getClass())))
            .map(
                bean -> {
  ...
  private List<SysPerm> getApiPerm(Class<?> clz, final String parentPval) {
    ...
    List<Method> apiMethods =
        MethodUtils.getMethodsListWithAnnotation(clz.getSuperclass(), RequiresPermissions.class);
