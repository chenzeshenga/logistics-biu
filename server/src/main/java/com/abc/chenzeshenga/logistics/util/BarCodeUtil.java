...
public class BarCodeUtil {
  ...
  private static void generate(String msg, OutputStream ous, String category) {
    ...
    final int dpi = 1024;
    // module宽度
    final double moduleWidth = UnitConv.in2mm(2.3f / dpi);
    // 配置对象
    abstractBarcodeBean.setModuleWidth(moduleWidth);
    abstractBarcodeBean.setHeight(7);
    abstractBarcodeBean.setFontSize(0.2);
  ...
  private static void generateQrCode(String content, OutputStream outputStream) {
    ...
    try {
      ...
      BitMatrix byteMatrix =
          qrCodeWriter.encode(content, BarcodeFormat.QR_CODE, size, size, hintMap);
