...
@PermInfo(value = "系统用户模块", pval = "a:sys:接口") @RestController @RequestMapping("/sys_user") public class SysUserController {
    ...
    @PermInfo("更新系统用户的角色") @RequiresPermissions("a:sys:user:role:update") @PatchMapping("/role")
    ...
    @PatchMapping("/pwd") public Json updatePwd(@RequestBody String body) {
    ...
    @PermInfo("查找系统用户的角色") @RequiresPermissions("a:sys:user:role:find") @GetMapping("/{uid}/roles")
